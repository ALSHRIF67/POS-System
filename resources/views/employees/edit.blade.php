        ...
        <div class="page-header mb-4 flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
                    <i class="fas fa-user-edit text-[#6C63FF]"></i>
                    تعديل بيانات الموظف
                </h2>
                <p class="text-sm text-gray-500 mt-1">تعديل معلومات الموظف: {{ $employee->name }}</p>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="role" 
                            ...
                            <option value="Chef" {{ (old('role', $employee->role) == 'Chef') ? 'selected' : '' }}>شيف</option>
                            <option value="Waiter" {{ (old('role', $employee->role) == 'Waiter') ? 'selected' : '' }}>جارسون</option>
                            <option value="Cashier" {{ (old('role', $employee->role) == 'Cashier') ? 'selected' : '' }}>كاشير</option>
                            <option value="Juice Maker" {{ (old('role', $employee->role) == 'Juice Maker') ? 'selected' : '' }}>عصاراتي</option>
                            <option value="Cleaner" {{ (old('role', $employee->role) == 'Cleaner') ? 'selected' : '' }}>عامل نظافة</option>
                            <option value="Accountant" {{ (old('role', $employee->role) == 'Accountant') ? 'selected' : '' }}>محاسب</option>
                            <option value="Manager" {{ (old('role', $employee->role) == 'Manager') ? 'selected' : '' }}>مدير</option>
                        </select>
                        @error('role')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        ...
                        <div class="flex gap-4">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" 
                                       ...
                                       {{ old('salary_type', $employee->salary_type) == 'daily' ? 'checked' : '' }}
                            ...
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" 
                                       ...
                                       value="monthly" 
                                       class="w-5 h-5 text-[#6C63FF] focus:ring-[#6C63FF]"
                                       {{ old('salary_type', $employee->salary_type) == 'monthly' ? 'checked' : '' }}
                                       onchange="toggleSalaryFields()"
                                       required>
                                <span>شهري</span>
                            </label>
                    ...
                    <div id="dailySalaryField" class="{{ old('salary_type', $employee->salary_type) == 'daily' ? '' : 'hidden' }}">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" 
                                   id="daily_salary" 
                                   name="daily_salary" 
                                   value="{{ old('daily_salary', $employee->daily_salary) }}"
                                   step="0.01"
                                   min="0"
                    ...
                    <div id="monthlySalaryField" class="{{ old('salary_type', $employee->salary_type) == 'monthly' ? '' : 'hidden' }}">
